...
function Answer() {
    ...
    //   console.log("logged data :", mockDataTeam);
      ...
      axios
        .post(`${baseUrl}/answer/getAll`, payload)
        .then((res) => {
    ...
    const columns = [
      ...
      // },
      {
        ...
        renderCell: ({ row }) => (
          <Button
            onClick={() => {
              const payload = { adminId: LoggedAdmin._id, answerId: row._id };
              // ${baseUrl}/admin/add-content`
              axios
                .post(`${baseUrl}/answer/delete`, payload)
                .then((res) => {
                  if (res.data.myType === "Success") {
                    toast.success(res.data.message);
    ...
    return (
      <Box m="20px">
        ...
        <Header title="Answers" subtitle="Managing the Answers" />
        <Box
          m="40px 0 0 0"
          height="75vh"
          sx={{
